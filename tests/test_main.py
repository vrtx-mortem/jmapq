...
class jmapqExtractionTests(unittest.TestCase):
    def test_sample_has_qualified_name(self) -> None:
        unit = jmapq.extract_unit(FIXTURES / "foo.java")
    ...
    def test_generics_and_throws(self) -> None:
        unit = jmapq.extract_unit(FIXTURES / "generics_throws.java")
    ...
    def test_nested_annotation_normalization(self) -> None:
        unit = jmapq.extract_unit(FIXTURES / "nested_annotations.java")
    ...
    def test_unknown_node_normalized_as_object(self) -> None:
        unknown = jmapq._normalize_node_value(object())
    ...
    def test_payload_is_json_serializable(self) -> None:
        payload = jmapq._build_payload([str(FIXTURES), str(FIXTURES / "foo.java")])
    ...
    def test_collect_java_files_with_glob(self) -> None:
        ...
        files = jmapq._collect_java_files([pattern])
    ...
    def test_builtin_query_resolution(self) -> None:
        query = jmapq._resolve_query("classes", None)
    ...
    def test_conflicting_query_options_raise(self) -> None:
        with self.assertRaises(ValueError):
            jmapq._resolve_query("classes", ".units")
    ...
    def test_custom_jq_query_via_subprocess(self) -> None:
        ...
        result = jmapq._run_jq(".units[].classes[].qualified_name", payload_json, True)
        self.assertEqual(result.strip(), "a.b.C")

    def test_media_types_builtin_query(self) -> None:
        if shutil.which("jq") is None:
            self.skipTest("jq not available")
        payload = jmapq._build_payload([str(FIXTURES / "foo.java")])
        payload_json = json.dumps(payload)
        query = jmapq.BUILTIN_QUERIES["media-types"][0]
        result = jmapq._run_jq(query, payload_json, False)
    ...
    def test_endpoint_params_text(self) -> None:
        ...
        text = jmapq._endpoint_params_text(params)
    ...
    def test_render_endpoints_plain_table(self) -> None:
        ...
        text = jmapq._render_endpoints_plain(json.dumps(endpoints))
    ...
    def test_parser_default_output_is_map_json(self) -> None:
        parser = jmapq._build_arg_parser()
    ...
    def test_get_payload_json_reads_existing_map_when_no_paths(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            map_path.write_text(json.dumps(payload), encoding=jmapq.ENCODING)
            loaded = jmapq._get_payload_json([], map_path)
    ...
    def test_get_payload_json_writes_map_when_paths_present(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            map_path = Path(tmp_dir) / "map.json"
            payload_json = jmapq._get_payload_json(
